...
Remember the budget is ${budget}
    ...
    # creates the SMTP session for sending the mail (gmail)
    ...
    # creates the SMTP session for sending the mail (hotmail)
    # session = smtplib.SMTP('smtp.live.com', 587)  # use Hotmail with port 587
    # session.starttls()  # enable security
    # session.login(sender_address, sender_pass)  # login with mail_id and password
    # text = message.as_string()
    # session.sendmail(sender_address, receiver_address, text)
    ...
    # creates the SMTP session for sending the mail (yahoo)
    session = smtplib.SMTP('smtp.mail.yahoo.com', 587)  # use yahoo with port 58
    session.starttls()  # enable security
    session.login(sender_address, sender_pass)  # login with mail_id and password
    text = message.as_string()
    session.sendmail(sender_address, receiver_address, text)
...
# Writing the allocations to a file
